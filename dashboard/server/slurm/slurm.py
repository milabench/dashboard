...
@contextmanager
def use_cluster(cluster: ClusterContext):
    # Set new value keeping a token to restore
    token = _cluster.set(cluster)
    try:
        yield _cluster.get()
    finally:
...
def _ssh_host():
    return _cluster.get().ssh
 
def _remote_folder():
    return _cluster.get().remote_folder

def _local_folder():
    return os.path.join(JOBRUNNER_LOCAL_CACHE, _cluster.get().local_folder)
...
def rsync_jobrunner_folder(timeout=5, force=False):
    with job_rsync_limiter(None) as can_run:
        ...
        rsync_cmd = ["rsync", "-az", f"{_ssh_host()}:{_remote_folder()}/", f"{_local_folder()}"]
...
def safe_job_path(*frags):
    return safe_path(_local_folder(), *frags)
...
def validate_jr_job_id(jr_job_id):
    ...
    safe_job_path(_local_folder(), jr_job_id)
...
def rsync_jobrunner_job(jr_job_id, timeout=5):
    ...
    with job_rsync_limiter(jr_job_id) as can_run:
        ...
        rsync_cmd = ["rsync", "-az", f"{_ssh_host()}:{_remote_folder()}/{jr_job_id}", f"{_local_folder()}"]
...
def slurm_integration(app, cache):
    ...
    def api_slurm_status():
        ...
        result = remote_command(_ssh_host(), f"sinfo -h")
    ...
    def api_slurm_persisted_limited(limit=None):
        ...
        try:
            ...
            job_dirs = []
            for item in os.listdir(_local_folder()):
    ...
    def api_slurm_all_jobs():
        ...
        try:
            # Get running and pending jobs using JSON format
            result = remote_command(_ssh_host(), "'sacct -u $USER --json'")
    ...
    def api_slurm_active_job_status(job_id):
        ...
        try:
            # Get running and pending jobs using JSON format
            result = remote_command(_ssh_host(), f"'squeue -u $USER -j {job_id} --json'")
    ...
    def api_slurm_old_job_status(job_id):
        ...
        try:
            # Get running and pending jobs using JSON format
            result = remote_command(_ssh_host(), f"'sacct -u $USER -j {job_id} --json'")
    ...
    def api_slurm_rerun(jr_job_id: str):
        ...
        old_local_dir = f"{_local_folder()}/{old_jr_job_id}"
        new_local_dir = f"{_local_folder()}/{new_jr_job_id}"
        new_remote_dir = f"{_remote_folder()}/{new_jr_job_id}"
        ...
        result = local_command("rsync", "-az", new_local_dir + "/", f"{_ssh_host()}:{new_remote_dir}")
        if not result['success']:
            return jsonify({'error': f'Failed to retrieve previous job command: {result["stderr"]}'}), 500

        # Submit job
        result = remote_command(_ssh_host(), new_cmd)
    ...
    def api_early_sync(jr_job_id, job_id):
        ...
        if os.path.exists(squeue_info):
            ...
            result = local_command(
                "rsync", "-az", f"{compute_node}:/tmp/{job_id}/cuda/results/runs/", f"{_local_folder()}/{jr_job_id}/runs"
            )
    ...
    def api_slurm_cancel(job_id):
        ...
        job_id = validate_slurm_job_id(job_id)
        try:
            result = remote_command(_ssh_host(), f'scancel {job_id}')
    ...
    def fetch_latest_job_acc(job_id):
        ...
        with _fetch_lock:
            ...
            result = remote_command(_ssh_host(), f'sacct --json -j {job_id}')
    ...
    def api_slurm_job_info(jr_job_id, job_id=None):
        ...
        try:
            ...
            # Try to get job logs using scontrol
            job_id = validate_slurm_job_id(job_id)
            result = remote_command(_ssh_host(), f'scontrol show job --json {job_id}')
            # result = remote_command(_ssh_host(), f'sacct --json -j {job_id}')
